...
int checkwin()
{
    int i, j, count = 0;
    int dir;
    int currentpiece = arrayForInnerBoardLayout[row][col]; // 当前位置的棋子
    // 遍历每个方向
    for (dir = 0; dir < 4; dir++)
    {
        count = 0;
...
int checkForbiddenMoves(int x, int y)
{
    ...
    // i1为正向横坐标，i11是i1后面一格，i2是负向横坐标，i22是i2前面一格
    // j1为正向纵坐标，j11是j1后面一格，j2是负向纵坐标，j22是j2前面一格
    int dir;              // 用于遍历方向
    int piecerow;         // 判断连子数
    int curpiece = BLACK; // 当前黑子
